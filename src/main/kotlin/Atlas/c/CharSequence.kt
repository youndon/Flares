...
private interface CharSequence {
    ...
    private fun attachmentOfTheCharSequence() {
        val charSequence = "Canada".asIterable()
...
//            this.subSequence(1..3) // -> tri
            ...
            filterTo(mutableListOf()) { it.isLowerCase() }
            filterNotTo(mutableListOf()) { it.isLetter() }
            filterIndexed { index, it -> it.isLowerCase() }
            filterIndexedTo(mutableListOf()) { index, c -> c.isLetter() }
            first()
            firstOrNull()
            find { it == 's' }
            findLast { it == 's' }
